...
if(!isset($_GET['view']))
{
	...
	$query3 = "SELECT * FROM taskinfo WHERE ((Type = 11 OR Type = 28) AND (Createdate > '$dateseven'))ORDER BY Createdate DESC";
	...
	while($row3 = mysql_fetch_array($result3))
	{
		...
		$type = $row3['Type'];
		mysql_select_db('homefree');
		$cusnum = $row3['CustomerNumber'];
		$datecreated = strtotime($row3['Createdate']);
		$ticketid = $row3['ticketNum'];
		$taskstatus = $row3['Status'];	
		mysql_select_db('work');
		...
		{			
			...
			if($count13 > 0)
			{
				...
				$tasktype = $row14['Type'];
				$display = '<tr><td class="SectionNav" width="749"><table><tr><td colspan="5"><b>'.$row4['FacilityName'].'</b>'.'<a href="' . 'rmareport.php?custnum='.$cusnum.'&view=bycus">('.  $cusnum . '</a>)'.'</td></tr>';
		...
		if(($datecreated > $currentmorning) && ($datecreated < $now))
		{
			$a = $a;
			if($a == 0)
			{
...
<?php
		...
		if(($datecreated > $onedaysago) && ($datecreated < $currentmorning))
		{
			$b = $b;
...
<?php
		...
		if(($datecreated > $twodaysago) && ($datecreated < $onedaysago))
		{
			$c = $c;
			if($c == 0)
			{			
...
<?php
		...
		if(($datecreated > $threedaysago) && ($datecreated < $twodaysago))
		{
			$d = $d;						
			if($d == 0)
			{				
...
<?php
		...
		if(($datecreated > $fourdaysago) && ($datecreated < $threedaysago))
		{
			$e = $e;						
			if($e == 0)
			{			
...
<?php
		...
		if(($datecreated > $fivedaysago) && ($datecreated < $fourdaysago))
		{
			$f = $f;						
			if($f == 0)
			{					
...
<?php
		...
		if(($datecreated > $sixdaysago) && ($datecreated < $fivedaysago))
		{
			$g = $g;						
			if($g == 0)
			{				
...
<?php
		...
		if(($datecreated > $sevendaysago) && ($datecreated < $sixdaysago))
		{
			$h = $h;
